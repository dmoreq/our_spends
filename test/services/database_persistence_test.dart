...
void main() {
  group('Database Persistence Tests', () {
    ...
    test('should persist expenses between app restarts', () async {
      ...
      // Insert expenses
      final expenseId1 = await databaseService.insertExpense(testExpense1);
      final expenseId2 = await databaseService.insertExpense(testExpense2);
      
      // Verify IDs were generated
      expect(expenseId1, isNotEmpty);
      expect(expenseId2, isNotEmpty);
      ...
      // Verify expenses are still available after "restart"
      expenses = await newDatabaseService.getExpenses();
      expect(expenses.length, 2);
      
      // Verify expense details were preserved
      final savedExpense1 = expenses.firstWhere((e) => e.id == expenseId1);
      expect(savedExpense1.amount, 50.0);
      expect(savedExpense1.category, 'Food & Dining');
      expect(savedExpense1.item, 'Lunch');
      
      final savedExpense2 = expenses.firstWhere((e) => e.id == expenseId2);
      expect(savedExpense2.amount, 30.0);
      expect(savedExpense2.category, 'Transportation');
      expect(savedExpense2.item, 'Bus fare');
    });
    
    test('should persist expense updates between app restarts', () async {
      // Setup SharedPreferences mock with empty initial values
      SharedPreferences.setMockInitialValues({});
    ...
    test('should persist expense deletion between app restarts', () async {
      // Setup SharedPreferences mock with empty initial values
      SharedPreferences.setMockInitialValues({});
